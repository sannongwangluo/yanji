...
一个给中文团队开会用的小工具：**开会时边说边出带说话人标签的文字，散会一键生成会议纪要并导出 Word + Markdown**，另附自动生成的「总结信息图」，本机装有 WPS/Word 时还会导出 PDF。
...
设置窗（填写/修改豆包语音与 DeepSeek 两个 API Key，并编辑热词表）：
...
```mermaid
flowchart LR
    A[开会<br/>开头报到 我是X] --> B[流式识别 + 说话人标签<br/>火山 2.0 · 200ms/包 · 二遍识别<br/>ssd 分离 + 报到映射 + 热词]
    B --> C[散会]
    C --> D[DeepSeek 生成纪要<br/>deepseek-flash<br/>长会议走滚动摘要]
    D --> E[docx + md<br/>总结信息图<br/>可选 PDF]
```
...
- **热词表**：`hotwords.txt`（一行一词，可在 GUI 设置窗里编辑）作为流式识别的热词语料下发，同时用作生成纪要时的写法白名单，人名/产品名/行话按正确写法出字；带 token 上限估算，超限直接拦住，避免服务端拒掉整次识别
- **断线不怕**：网络抖动指数退避自动重连（最多 5 次）；每句转写实时落盘 `日志/transcript_*.jsonl`，崩机不丢
- **长会议滚动摘要**：会议满 30 分钟后，每 20 分钟在后台预写一轮摘要；散会时只把摘要之后的新转写发给模型续写，出稿更快也不丢内容（每轮摘要同时落盘 `日志/digest_*.md`，崩机还有最新摘要可人工补救）
- **出什么**：DeepSeek 按发言人轮次整理，产出 docx 会议纪要（三档排版：标题/正文/列表），docx + md 同名成对导出；再用 Pillow 画一张「总结信息图」PNG，插入 docx/md；本机装了 WPS/Word 时额外导出一份 PDF（没装就跳过，不影响出稿）
- **单实例守卫**：重复双击只把已有窗口提到前台，不产生第二个实例；上次崩溃残留的僵尸进程会被安全回收

## 快速开始（三步）

1. 启动程序，点 **【开始录音】**（手机、全向麦或电脑麦克风都行）
2. 开会。**开头先每人依次说一句「我是+姓名」**，比如「我是张三」——窗口里实时滚动出 `张三：……` 的文字
3. 散会时点 **【结束并生成纪要】**：转写稿开会时已经实时攒好，直接交给 DeepSeek，通常一两分钟出 docx，自动打开「输出」文件夹（Word + Markdown + 总结信息图；装有 WPS/Word 时还有 PDF）
...
容易被听错的人名、产品名、行话，写进 `hotwords.txt`（一行一词），在 GUI「设置…」窗里编辑即可。这份词表会作为流式识别的热词下发，同时用作生成纪要时的写法白名单。**别贪多**——服务端对词表长度有上限，程序也会在保存时拦住超限的词表。
...
依赖：按 `requirements.txt` 安装（Python 3.11+；`pywin32` 与 `Pillow` 是 PDF 导出与总结信息图的可选依赖）。
...
```bash
python -m unittest discover -s tests         # 351 个用例
```
...
| 文件 | 作用 |
| --- | --- |
| `会议记录.py` / `yanji.py` | GUI 入口（中文 / 英文启动器） |
| `pipeline.py` | 管线编排：映射 → 纪要 → docx/md/PDF |
| `streaming_asr.py` | 流式识别会话（WebSocket、重连、热词） |
| `asr_client.py` | 备用文件识别路径（TOS 上传 + 轮询） |
| `speaker_map.py` | 「我是X」报到 → 说话人姓名映射 |
| `minutes_llm.py` | DeepSeek 纪要 + 滚动摘要 prompt |
| `digest.py` | 长会议滚动摘要状态机 |
| `hotwords.py` | `hotwords.txt` 读写 + token 上限守卫 |
| `infographic.py` | 总结信息图绘制（Pillow） |
| `docx_writer.py` | docx/md 导出 + 信息图插入 + docx→PDF |
| `recorder.py` | 麦克风录音（sounddevice） |
| `config_loader.py` | `config.toml` 读写 |
| `gen_manual_docx.py` | 手写排版 docx 辅助 |
| `tests/` | 单元测试 |
...
5. **可选加料是 best-effort**：总结信息图依赖系统微软雅黑字体，PDF 导出依赖本机装有 WPS 或 Word；缺任意一项就自动跳过，不影响 docx/md 出稿
...
- **数据在自己硬盘上**：录音、转写、纪要、热词表全在本地，只有 API 调用出门——不存在消费级云账号里
...
- 纪要生成（DeepSeek）：每次几分钱
- 总结信息图与 PDF 导出完全在本机完成，不产生额外 API 费用
...
**Q：某个名字总是识别成错别字？**
A：把它加进热词表——GUI「设置…」→ 热词 → 保存，或直接编辑 `hotwords.txt`（一行一词）。这份词表既用于流式识别的热词下发，也用作生成纪要时的写法白名单。只放最容易被听错的几个词就够了。
...
**Q：生成的纪要不见了？**
A：在程序所在文件夹的「输出」文件夹里，文件名 `会议纪要_年月日_时分.docx`（同名还有 `.md` 和 `_总结图.png`）。本机装了 WPS/Word 时还会多一个同名 `.pdf`。

## 路线图

- **v2 本地隐私引擎**：SenseVoice 本地识别——整段识别先行、说话人分离后置
- **更多导出格式**：经本机 WPS/Word 的 PDF 导出已内置，后续再补其他格式
